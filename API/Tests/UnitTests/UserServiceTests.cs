...
namespace AutomationFramwork.API.Tests.UnitTests
{
    ...
    public class UserServiceTests
    {
        ...
        [Test]
        public async Task GetUserAsync_ShouldReturnUser_WhenIdIsValid()
        {
            // Arrange
            var userId = 1;
            var expectedUser = new User { Id = userId, Name = "John Doe" };
            _mockUserApiClient.Setup(api => api.GetUserAsync(userId)).ReturnsAsync(expectedUser);
        ...
        [Test]
        public async Task CreateUserAsync_ShouldReturnCreatedUser_WhenUserIsValid()
        {
            // Arrange
            var newUser = new User { Name = "Jane Doe" };
            var expectedUser = new User { Id = 1, Name = "Jane Doe" };
            _mockUserApiClient.Setup(api => api.CreateUserAsync(newUser)).ReturnsAsync(expectedUser);
        ...
        public async Task UpdateUserAsync_ShouldReturnUpdatedUser_WhenUserIsValid()
        {
            ...
            _mockUserApiClient.Setup(api => api.UpdateUserAsync(existingUser)).ReturnsAsync(updatedUser);
        ...
        [Test]
        public async Task DeleteUserAsync_ShouldReturnTrue_WhenIdIsValid()
        {
            // Arrange
            var userId = 1;
            _mockUserApiClient.Setup(api => api.DeleteUserAsync(userId)).ReturnsAsync(true);
